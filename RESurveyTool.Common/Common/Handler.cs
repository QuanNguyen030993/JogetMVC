...
namespace ERPCore.Common.Common
{
    public static class Handler
    {
        public static void ErrorException(Exception exception, string blobpath)
        {
            try
            {
                Serilog.Log.Error(exception, "Library exception occurred. BlobPath: {BlobPath}", blobpath);
            }
            catch (Exception ex)
            {
                Console.WriteLine($"Failed to log exception in Handler: {ex.Message}. Original exception: {exception.Message}");
            }
        }
    }
}
