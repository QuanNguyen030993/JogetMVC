...
var treefilter = function(el, options) {
	...
	var init = function() {
		...
		el.find("li.treeview-menu-general > div:only-child").parent().removeClass("tf-child-true");
		el.find("li.treeview-menu-general > div:only-child").parent().addClass("tf-child-false");
	...
	var searcher = function() {
		$(plugin.settings.searcher).keyup(function() {
			if ($(this).val().length == 0) {
				...
				plugin.el.find(".tf-search-result").show();
			}
		});
