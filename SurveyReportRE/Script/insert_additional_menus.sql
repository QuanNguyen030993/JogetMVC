...
BEGIN TRY
    ...
    -- Signed-back Inbox
    IF NOT EXISTS (SELECT 1 FROM Menu WHERE ParentId = @QuotationParentId AND Caption = 'Signed-back Inbox' AND Deleted = 0)
    BEGIN
        INSERT INTO Menu (
            ...
            'SignedBackQuotation', 'Signed-back Inbox', NULL, @QuotationParentId, NULL, '/Business/Form/SignedBackQuotation', 1, 'api', 'Quotation',
            'quan.nh', GETDATE(), NULL, NULL, 6, 'Management', 0, '', NULL, NEWID(), '', GETDATE()
        );
        PRINT 'Inserted menu: Signed-back Inbox';
    END
