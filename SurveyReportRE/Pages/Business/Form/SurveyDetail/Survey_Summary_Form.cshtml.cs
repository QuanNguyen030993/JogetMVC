...
{
    public class Survey_Summary_FormModel : PageModel
    {
        //private readonly ILogger<Survey_Summary_FormModel> _logger;
        ...
        public Survey_Summary_FormModel(ILogger<Survey_Summary_FormModel> logger, IConfiguration config)
        {
            //_logger = logger;
        }
        public async void OnGet(int? pageNum, int? refPageNum,string jsonConfig) { ViewData[nameof(JsonConfig)] = jsonConfig;

            ModelName = nameof(Summary);
            FKModelName = nameof(Survey);
            SchemeModelName = nameof(Summary);
