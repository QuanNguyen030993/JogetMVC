...
{
    public class Survey_Appendix_FormModel : PageModel
    {
        //private readonly ILogger<Survey_Appendix_FormModel> _logger;
        ...
        public Survey_Appendix_FormModel(ILogger<Survey_Appendix_FormModel> logger, IConfiguration config)
        {
            //_logger = logger;
        }
        public async void OnGet(int? pageNum, int? refPageNum,string jsonConfig) { ViewData[nameof(JsonConfig)] = jsonConfig;
            ...
            ModelName = nameof(Appendix);
            FKModelName = nameof(Survey);
            SchemeModelName = nameof(Appendix);
