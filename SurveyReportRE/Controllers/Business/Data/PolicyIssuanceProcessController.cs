...
public class PolicyIssuanceProcessController : BaseControllerApi<PolicyIssuanceProcess>
{
    private readonly IBaseRepository<PolicyIssuanceProcess> _BaseRepository;
    ...
    public PolicyIssuanceProcessController(IBaseRepository<PolicyIssuanceProcess> BaseRepository, IConfiguration config, IHttpContextAccessor httpContextAccessor, ILogger<PolicyIssuanceProcess> logger) : base(BaseRepository, httpContextAccessor)
    {
    ...
    public async Task PullData()
    {
        ...
        //var list = ConvertToPolicyIssuanceProcessList(obj);
        //await BulkInsertPolicyIssuanceProcessAsync(list);
    }
    public async Task BulkInsertPolicyIssuanceProcessAsync(List<PolicyIssuanceProcess> data)
    {
        var dt = new DataTable();

        // Khởi tạo cột (phải khớp DB)
        foreach (var prop in typeof(PolicyIssuanceProcess).GetProperties())
        {
        ...
        {
            DestinationTableName = "dbo.PolicyIssuanceProcess", // Đảm bảo đúng tên bảng
            BulkCopyTimeout = 60
        };
    ...
    public static List<PolicyIssuanceProcess> ConvertToPolicyIssuanceProcessList(List<Dictionary<string, object>> rawData)
    {
        var result = new List<PolicyIssuanceProcess>();

        foreach (var dict in rawData)
        {
            var obj = new PolicyIssuanceProcess();
            foreach (var prop in typeof(PolicyIssuanceProcess).GetProperties())
            {
                var key = prop.Name;
                if (key == "Id") continue;
