...
public class RecordWorkflowController : BaseControllerApi<RecordWorkflow>
{
    private readonly IBaseRepository<RecordWorkflow> _BaseRepository;
    ...
    public RecordWorkflowController(IBaseRepository<RecordWorkflow> BaseRepository, IConfiguration config, IHttpContextAccessor httpContextAccessor, ILogger<RecordWorkflow> logger) : base(BaseRepository, httpContextAccessor)
    {
    ...
    public async Task BulkInsertRecordWorkflowAsync(List<RecordWorkflow> data)
    {
        ...
        foreach (var prop in typeof(RecordWorkflow).GetProperties())
        {
        ...
        {
            DestinationTableName = "dbo.RecordWorkflow", // Đảm bảo đúng tên bảng
            BulkCopyTimeout = 60
        };
    ...
    public static List<RecordWorkflow> ConvertToRecordWorkflowList(List<Dictionary<string, object>> rawData)
    {
        var result = new List<RecordWorkflow>();

        foreach (var dict in rawData)
        {
            var obj = new RecordWorkflow();
            foreach (var prop in typeof(RecordWorkflow).GetProperties())
            {
                var key = prop.Name;
                if (key == "Id") continue;
