...
public class LineController : BaseControllerApi<Line>
{
    private readonly IBaseRepository<Line> _BaseRepository;
    private readonly IBaseRepository<Utility> _utilityRepository;
    private readonly IConfiguration configuration;

    public LineController(IBaseRepository<Line> BaseRepository, IConfiguration config, IHttpContextAccessor httpContextAccessor) : base(BaseRepository, httpContextAccessor)
    {
    ...
    {// Use blog settings while override this method instead
        ...
        if (file != null && file.Length > 0)
        {
            using (var ms = new MemoryStream())
            {
                ...
                var row2 = rows[1]; // property name (LineId, TSCode, ...)
                                    // rows[2..] là data
                ...
                // property name theo cột: colIndex -> "LineId"...
                ...
                var propCache = typeof(Line)
                    .GetProperties(BindingFlags.Public | BindingFlags.Instance)
                    .ToDictionary(p => p.Name, p => p, StringComparer.OrdinalIgnoreCase);


                //var settersByCol = new Dictionary<int, Action<Line, string>>(colToProp.Count);
                ...
                //{
                //    await _BaseRepository.InsertData(Line);
                //}
    ...
    private static void SetPropertyValue(Line dto, PropertyInfo prop, string? raw)
    {
        var s = (raw ?? "").Trim();
