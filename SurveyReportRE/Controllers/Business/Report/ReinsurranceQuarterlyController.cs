...
public class ReinsurranceQuarterlyController : BaseControllerApi<ReinsurranceQuarterly>
{
    private readonly IBaseRepository<ReinsurranceQuarterly> _BaseRepository;
    ...
    public ReinsurranceQuarterlyController(IBaseRepository<ReinsurranceQuarterly> BaseRepository, IConfiguration config, IHttpContextAccessor httpContextAccessor, ILogger<ReinsurranceQuarterly> logger) : base(BaseRepository, httpContextAccessor)
    {
    ...
    public async Task BulkInsertReinsurranceQuarterlyAsync(List<ReinsurranceQuarterly> data)
    {
        ...
        foreach (var prop in typeof(ReinsurranceQuarterly).GetProperties())
        {
        ...
        {
            DestinationTableName = "dbo.ReinsurranceQuarterly", // Đảm bảo đúng tên bảng
            BulkCopyTimeout = 60
        };
    ...
    public static List<ReinsurranceQuarterly> ConvertToReinsurranceQuarterlyList(List<Dictionary<string, object>> rawData)
    {
        var result = new List<ReinsurranceQuarterly>();

        foreach (var dict in rawData)
        {
            var obj = new ReinsurranceQuarterly();
            foreach (var prop in typeof(ReinsurranceQuarterly).GetProperties())
            {
                var key = prop.Name;
                if (key == "Id") continue;
